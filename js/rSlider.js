(function () {
	...
	RS.prototype.setValues = function (start, end) {
		...
		if (this.conf.range) {
			if (this.conf.tooltip) {
				var temp_start = this.conf.values[this.values.start];
				temp_start = temp_start.toString();
				if(temp_start.endsWith('000'))
				{
					temp_start = temp_start.slice(0, -3);
					temp_start = temp_start + 'k';
				}
				var temp_end = this.conf.values[this.values.end];
				temp_end = temp_end.toString();
				if(temp_end.endsWith('000'))
				{
					temp_end = temp_end.slice(0, -3);
					temp_end = temp_end + 'k';
				}
				this.tipL.innerHTML = temp_start;
				this.tipR.innerHTML = temp_end;
				// this.tipL.innerHTML = this.conf.values[this.values.start];
				// this.tipR.innerHTML = this.conf.values[this.values.end];
			}
			this.input.value = this.conf.values[this.values.start] + ',' + this.conf.values[this.values.end];
			this.pointerR.style.left = (this.values.end * this.step - (this.pointerWidth / 2)-5) + 'px';
		}
		else {
			if (this.conf.tooltip)
